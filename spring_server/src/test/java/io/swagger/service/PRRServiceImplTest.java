...
public class PRRServiceImplTest {
    ...
    public void getPRRByCode() {
        ...
        List<PRR> prrList = prrServImpl.getPRRByCode(drugCode, outcomeCode);
        assertTrue(prrList.size() > 0);
        PRR prr = prrList.get(0);
    ...
    public void getPRRByName() {
        ...
        List<PRR> prrList = prrServImpl.getPRRByName(drugName, outcomeName);
        assertTrue(prrList.size() > 0);
        PRR prr = prrList.get(0);
    ...
    public void getPRRByDateCode() {
        ...
        List<PRR> prrList = prrServImpl.getPRRByDateCode(drugCode, outcomeCode,startDate,endDate);
        assertTrue(prrList.size() > 0);
        PRR prr = prrList.get(0);
        assertSame(prr.id, prrCode.id);
    ...
    public void getPRRByDateName() {
        ...
        List<PRR> prrList = prrServImpl.getPRRByDateName(drugName, outcomeName,startDate,endDate);
        assertTrue(prrList.size() > 0);
        PRR prr = prrList.get(0);
        assertSame(prr.id, prrName.id);
