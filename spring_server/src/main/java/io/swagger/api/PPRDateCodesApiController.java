...
public class PPRDateCodesApiController implements PPRDateCodesApi {
    ...
    public ResponseEntity<List<PRR>> pprDateCodes(@NotNull @ApiParam(value = "RxNorm drug code for search", required = true) @Valid @RequestParam(value = "drugCode", required = true) String drugCode,@NotNull @ApiParam(value = "SNOMEDCT outcome code for search", required = true) @Valid @RequestParam(value = "outcomeCode", required = true) String outcomeCode,@NotNull @ApiParam(value = "Start date YYYYMMDD", required = true) @Valid @RequestParam(value = "startDate", required = true) String startDate,@NotNull @ApiParam(value = "End date YYYYMMDD", required = true) @Valid @RequestParam(value = "endDate", required = true) String endDate) {
        ...
        List<PRR> prr = prrService.getPRRByDateCode(DrugCode, OutcomeCode, StartDate, EndDate);
        if (!prr.isEmpty()) {
