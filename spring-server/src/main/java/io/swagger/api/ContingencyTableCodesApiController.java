...
public class ContingencyTableCodesApiController implements ContingencyTableCodesApi {
    ...
    public ResponseEntity<List<ContingencyTable>> contingencyTableCodes(@NotNull @ApiParam(value = "RxNorm drug code for search", required = true) @Valid @RequestParam(value = "drugCode", required = true) String drugCode,@NotNull @ApiParam(value = "SNOMEDCT outcome code for search", required = true) @Valid @RequestParam(value = "outcomeCode", required = true) String outcomeCode) {
        String accept = request.getHeader("Accept");
        int DrugCode = Integer.parseInt(drugCode);
        int OutcomeCode = Integer.parseInt(outcomeCode);
        List<ContingencyTable> contingencyTable = contService.getContingencyTableByCode(DrugCode, OutcomeCode);
